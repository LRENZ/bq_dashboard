...
const projectId = process.env.PROJECT_ID || "bigquery-2024";
...
function buildTransitionModel(paths) {
  ...
  for (const [from, row] of transitionCounts.entries()) {
    ...
    transitions.set(from, new Map([...row.entries()].map(([to, count]) => [to, count / total])));
  }
...
function calculateWeights(paths) {
  ...
  return effects.map((row) => ({ ...row, weight: row.removalEffect / totalEffect }));
}
...
async function loadPaths(weightDate) {
  ...
  const [rows] = await bigquery.query({
    location: "US",
    query: `
      SELECT ARRAY_AGG(channel ORDER BY event_date) AS channels
      FROM ${sourceTable}
...
async function mergeWeights(weightDate, windowStart, weights) {
  ...
  const rowsSql = weights.map((row) => `
    SELECT DATE '${weightDate}' AS weight_date,
  ...
  await bigquery.query({
    ...
    query: `
      ...
      ON target.weight_date = source.weight_date AND target.channel = source.channel
...
exports.runDailyMarkov = async (req, res) => {
  try {
    const weightDate = req.query.targetDate || req.body?.targetDate || defaultTargetDate();
    ...
    res.status(200).json({
      ...
      weightSum: weights.reduce((sum, row) => sum + row.weight, 0),
    });
  } catch (error) {
    console.error(error);
